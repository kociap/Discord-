...
UIPalette DriftUIStyle::buildDarkPalette() {
    UIPalette ret;
    ...
    buttonGradient.setColorAt(0.0, QColor(64, 64, 64));
    buttonGradient.setColorAt(1.0, QColor(55, 55, 55));

    buttonHoverGradient.setColorAt(0.0, QColor(55, 55, 55));
    buttonHoverGradient.setColorAt(1.0, QColor(64, 64, 64));
...
UIPalette DriftUIStyle::buildLightPalette() {
    return {};
}
...
void DriftUIStyle::drawPrimitive(PrimitiveElement pe, const QStyleOption* opt, QPainter* p, const QWidget* widget) const {
    ...
    case PE_Frame: {
        ...
        p->fillPath(shape, QColor(47, 47, 47));
        p->strokePath(shape, QPen(QColor(21, 21, 21)));
    ...
    case PE_FrameMenu: {
        ...
        QPainterPath shape;
        shape.addRoundedRect(opt->rect.marginsRemoved(QMargins(1, 1, 1, 1)), 3, 3);
    ...
    case PE_PanelItemViewRow: {
        ...
        p->setBrush(m_pal[UIColor::WindowBG]);
        p->drawRect(opt->rect);
...
void DriftUIStyle::drawControl(ControlElement element, const QStyleOption* opt, QPainter* p, const QWidget* widget) const {
    ...
    case CE_PushButton: {
        ...
        QPainterPath shape;
        shape.addRoundedRect(QRect(x, y, w, h).marginsRemoved(QMargins(1, 1, 1, 1)), 4, 4);

        p->fillPath(shape, fillGradient);
        p->strokePath(shape, QPen(QColor(21, 21, 21), 2));

        p->drawText(rOpt->rect.center() + QPoint(rOpt->fontMetrics.width(rOpt->text) / -2, (rOpt->fontMetrics.capHeight() / 2) + 2), rOpt->text);
    ...
    case CE_MenuBarEmptyArea: {
        ...
        QPainterPath shape;
        shape.addRoundedRect(opt->rect.marginsRemoved(QMargins(5, 5, 5, 5)), 3, 3);
    ...
    case CE_MenuItem: {
        ...
        if (highlight) {
            QPainterPath shape;
            shape.addRoundedRect(rOpt->rect.marginsRemoved(QMargins(1, 1, 1, 1)), 2, 2);
            p->fillPath(shape, QColor(25, 94, 231));
        } else {
            QPainterPath shape;
            shape.addRoundedRect(rOpt->rect, 3, 3);
        ...
        p->drawText(textPoint, tr(firstPart.toUtf8().constData()));
        QColor blended = opt->palette.color(QPalette::BrightText);
        blended.setAlpha(100);
        p->setPen(blended);
        p->drawText(textPoint + QPoint(opt->fontMetrics.width(firstPart), 0), secondPart);
